...
HTTP_Request *HTTP_Request::receive(TCP_Socket &sock) {
    ...
    // Read lines until an empty line is found (consecutive CLRF)
...
HTTP_Request* HTTP_Request::parse(const char* data, unsigned length) {
    ...
    if (version_pos != string::npos) {
        request->set_url(request_line.substr(url_pos + 1,
            version_pos - url_pos - 1));
...
void HTTP_Request::set_url(const string& url) {
    this->url = url;
}
