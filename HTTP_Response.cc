...
// Modified again for CSE422 FS13
...
HTTP_Response::HTTP_Response(unsigned code, const string& status_desc,
    ...
    set_header_field("Server", "MSU/CSE422/FS20");
...
/**
 * Builds and sets the status_desc member according to the standard.
 */
...
/**
 * Creates a time string of the current time according to the standard.
 *
 * @return Time string
 */
string HTTP_Response::build_time() const {
    // format a time
    time_t t;
    struct tm *ts;
    char result[38];
    ...
    // format the time string according to the spec,
...
void HTTP_Response::append_content(const std::string& new_content) {
    content += new_content;
}

void HTTP_Response::clear_content() {
    content.clear();
}
...
int HTTP_Response::send(TCP_Socket &sock) {
    string outgoing_buffer;
    print(outgoing_buffer);
    outgoing_buffer.append(content);

    return sock.write_string(outgoing_buffer);
}
